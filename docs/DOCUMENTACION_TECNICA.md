...
`guardar_con_formato(path, hojas)` crea un único `xlsxwriter.Workbook(path, {"constant_memory": True})` para **todas** las hojas, sean `pd.DataFrame` (`_escribir_hoja_dataframe`) o de posición libre (`_escribir_hoja_libre`). `constant_memory` transmite cada fila directamente a disco sin mantener el worksheet completo en memoria — igual que el modo `write_only` de openpyxl (motor usado hasta esta ronda de optimización, ver sección 6c), pero midiendo 2.52x más rápido en un benchmark de validación sobre datos sintéticos del shape real (55 columnas, ~1.75M filas): 348s con XlsxWriter vs 878s con openpyxl `write_only`. `wb.use_zip64()` se habilita en cada workbook — sin esto, XlsxWriter lanza `FileSizeError` en cuanto el `.xlsx` comprimido supera ~4GB, algo real a esta escala (varios millones de filas × 55 columnas).

La contrapartida de `constant_memory` es la misma que tenía `write_only` en openpyxl: **no admite acceso aleatorio a celdas** ni volver a tocar una fila ya escrita, y además exige que las celdas de una misma fila se escriban en **orden de columna estrictamente creciente**. Por eso:
- Los formatos (encabezado: relleno `1F4E78`, negrita, blanco, centrado; moneda: `#,##0.00`) se crean **una sola vez** con `workbook.add_format(...)` al inicio de `guardar_con_formato`, y se reutiliza el mismo objeto `Format` en cada celda que lo necesite — a diferencia del motor anterior, que envolvía cada celda formateada en un `WriteOnlyCell` nuevo.
- `freeze_panes`/`set_column` no tienen la restricción de orden que sí tenía `column_dimensions` en openpyxl `write_only` (podían fijarse solo antes del primer `append`); en XlsxWriter se pueden llamar en cualquier momento antes de `workbook.close()`, pero por claridad se siguen fijando antes de escribir filas.
- **`_escribir_hoja_libre`** arma una grilla completa en memoria (`list[list]`, de la fila 1 a la última fila usada, columna A a la última columna usada, con `None` en las posiciones sin celda definida) a partir de la lista de celdas de posición libre, y la vuelca fila por fila con `worksheet.write_row(...)` (o `write()` celda por celda cuando la fila tiene alguna celda en negrita) — las columnas conservan su posición real (D, E, F...) para que la plantilla replicada (ver `_layout_valor_cinta` en `src/reglas.py`) no cambie de posición.

**`_escribir_hoja_dataframe`**: `_clasificar_columnas(df)` determina, por nombre de columna, qué formato de número aplica — solo `"monto"`/`"valor"` en el nombre (minúsculas) → `#,##0.00`; **las columnas `"fecha"` ya no reciben ningún `number_format`** (ver nota más abajo, era costo sin beneficio visual). `_anchos_columnas(df)` calcula el ancho de cada columna con una **muestra** de las primeras `_FILAS_MUESTRA_ANCHO` (2000) filas (`df.head(2000)`), no con el DataFrame completo — con hojas de cientos de miles o millones de filas (ej. un consolidado de 3-4 archivos planos de ~1M filas cada uno), recorrer las 55 columnas completas solo para un ancho visual aproximado es un costo real; el ancho es cosmético, nunca afecta los datos. El `fillna("")` antes de `astype(str)` es necesario porque, en pandas 3.x (la versión instalada en este entorno), `astype(str)` sobre un `NaN` de una columna `float64` (ej. `MONTO-1` cuando `pd.to_numeric(..., errors="coerce")` no pudo convertir el valor) **ya no produce la cadena `"nan"`** como en pandas < 3 — deja el `NaN` como `float`, y `.map(len)` revienta con `TypeError: object of type 'float' has no len()`; esto rompía la generación de cualquier hoja con al menos una fila cuyo `MONTO-1` no se pudo convertir, un caso real y común (filas con el campo en blanco). Los valores de fila se preparan con `df.astype(object).where(df.notna(), None).values.tolist()` (`NaN`/`NaT` → `None`, porque XlsxWriter no acepta `NaN` como valor de celda). Con `indices_formato` (precalculado una sola vez por hoja, normalmente solo `MONTO-1`) acotado a 1-2 columnas de 55, cada fila se escribe en tramos: `worksheet.write_row(fila, inicio, bloque_sin_formato)` para los tramos sin formato y `worksheet.write(fila, idx, valor, fmt_moneda)` para la celda puntual con formato, en orden de columna creciente — en vez de recorrer las 55 columnas de cada fila para decidir esto.

> **Nota de rendimiento — aplicada**: las columnas `"fecha"` recibían `number_format = "DD/MM/YYYY"` aunque `lector.py`/`reglas.py` nunca convierten esos campos a un tipo fecha real (siguen siendo texto, ej. `"20260813"`) — Excel ignora `number_format` sobre celdas de texto, así que ese formato no tenía ningún efecto visual, pero igual costaba tiempo de escritura. Se detectó al medir el benchmark de la sección 6c (~1.75M filas combinadas en Archivo 2 tardaron 690s en escribirse con el motor anterior) y se eliminó como parte del cambio de motor a XlsxWriter (sección 6d) — si en el futuro se necesita mostrar la fecha con formato real en Excel, primero hay que convertir la columna a un tipo fecha/datetime real en `reglas.py`, y solo entonces reincorporarla a `_clasificar_columnas`.
...
Esta ronda de optimización dejó el motor de escritura (openpyxl) sin tocar; el paso siguiente, forzado por que 690s (~11.5 min) para un solo archivo seguía siendo demasiado lento en producción, fue reemplazar el motor completo — ver sección 6d.

## 6d. Cambio de motor de escritura: openpyxl → XlsxWriter

Con archivos reales de ~1.2 millones de filas, el usuario reportó que generar Archivo 2 tardaba **~11 minutos**, consistente con el benchmark de la sección 6c (~690s para ~1.75M filas de salida) — y pidió explícitamente la opción más rápida posible, ya que se procesan hasta 3-4 archivos por corrida. Con el parseo ya optimizado a ~59s para 3M filas combinadas (sección 6b/6c), el cuello de botella real pasó a ser exclusivamente la escritura del `.xlsx`.

**Benchmark de validación** (antes de comprometerse a la reescritura completa): DataFrame sintético del shape real (55 columnas, `MONTO-1`/`MONTO-2` numéricas con `NaN`, ~1.75M filas — el tamaño de salida de Archivo 2 en el benchmark de la sección 6c), escrito con ambos motores:

| Motor | Tiempo |
|---|---|
| openpyxl `write_only` (con formato de fecha, como antes) | 877.5s |
| XlsxWriter `constant_memory` (sin formato de fecha, ver sección 6) | 347.9s |
| **Mejora** | **2.52x** |

Con una mejora de más de 2x confirmada, se procedió a reescribir `src/exportador.py` sobre XlsxWriter en vez de openpyxl (ver sección 6 para el detalle de la implementación) y a retirar el formato de fecha (dead weight confirmado, ver nota en sección 6).

**Benchmark A/B controlado con datos reales**: un primer intento de medir el pipeline completo (parseo + reglas + escritura) de punta a punta arrojó números sin sentido (parseo de 3M filas en ~67 min, reglas de negocio en ~3.7h) — la máquina se suspendió durante la corrida (la sesión cruzó medianoche) y `time.perf_counter()` contó ese tiempo de suspensión como si fuera trabajo real. Se descartó esa corrida y se repitió con una comparación A/B más controlada: mismos datos reales de producción, mismo DataFrame y las reglas de negocio (`generar_archivo_2`) aplicadas **una sola vez**, comparando después solo el paso de escritura con ambos motores, en el mismo proceso, uno después del otro — así se aísla el efecto del motor de cualquier ruido de la máquina o de datos distintos entre corridas.

| Escenario | openpyxl `write_only` | XlsxWriter `constant_memory` | Mejora |
|---|---|---|---|
| Sintético, 55 cols, ~1.75M filas (Tarea 1, validación aislada) | 877.5s | 347.9s | 2.52x |
| Real, hoja "RECH EP" filtrada de datos de producción, 289,571 filas | 153.0s | 94.4s | 1.62x |

La mejora real medida con datos de producción (**1.62x**) es menor que la del benchmark sintético aislado (2.52x) — los campos de texto reales (ej. `PLANO`, 125 caracteres) son más largos y variados que las cadenas cortas aleatorias usadas en el benchmark sintético, lo que reduce la ventaja relativa de XlsxWriter. Aun así, sigue siendo una mejora real y reproducible, no un artefacto de medición.

**Extrapolación al caso reportado por el usuario** (archivo de ~1.2M filas, Archivo 2 tardando ~11 min): con 1.62x de mejora, el tiempo esperado de escritura baja a aproximadamente **~6.5-7 min** — una reducción real, aunque no del orden de magnitud que el benchmark sintético inicial hacía esperar.

`openpyxl` dejó de ser una dependencia del proyecto — solo se usa `XlsxWriter` para escritura de Excel (ver sección 7).
...
| `XlsxWriter` | `src/exportador.py` | `Workbook(path, {"constant_memory": True})`, `add_format`, `write_row`/`write` |
| `tkinter` (stdlib, import opcional con fallback) | `src/validador.py` | Diálogo de selección múltiple de archivos |

Resto: módulos estándar (`pathlib`, `datetime`, `re`, `typing`, `functools`, `logging`, `os`). No existe `requirements.txt`, `pyproject.toml` ni `.gitignore` en la raíz — las dependencias (`pandas`, `xlsxwriter`) deben instalarse manualmente en el entorno Python que ejecuta el programa (`pip install pandas xlsxwriter`; `tkinter` viene con la instalación estándar de Python en Windows). `openpyxl` fue el motor de escritura hasta la ronda de optimización de la sección 6d y ya no es una dependencia del proyecto.
...
- **Formato de moneda por heurística de nombre de columna**: cualquier columna futura que contenga `"monto"`/`"valor"` en su nombre (minúsculas) hereda el formato automáticamente, sin tocar `src/exportador.py`. Las columnas `"fecha"` ya no reciben formato (ver sección 6) porque siguen siendo texto sin convertir.
